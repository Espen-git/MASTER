classdef modified_capon_minimum_variance < postprocess
    ...
    %   Modified by: Espen Lønes <espen.lones@gmail.com>  
    %
    %   $Last updated: 2023/04/04$
    
    %% constructor
    methods (Access = public)
        function h=modified_capon_minimum_variance()
    ...
    methods
        function [output]=go(h, name, call_type, Ria_type) % call_type: 0=normal, 1=save R, 2=load R_inv       
...
%                     
                        ...
                        image = capon_minimum_variance_implementation(h,data_cube,apod_matrix,['1/1'], n_frame, name, call_type, Ria_type);
                ...
                case dimension.transmit
                    ...
                    for n_frame = 1:h.input.N_frames
                        for n_channel = 1:h.input.N_channels
                            ...
                            image = capon_minimum_variance_implementation(h,data_cube,apod_matrix,[num2str(n_channel),'/',num2str(h.input.N_channels)], n_frame, name, call_type, Ria_type);
                ...
                case dimension.receive
                    ...
                    for n_frame = 1:h.input.N_frames
                        for n_wave = 1:h.input.N_waves
                            ...
                            image = capon_minimum_variance_implementation(h,data_cube,apod_matrix,[num2str(n_wave),'/',num2str(h.input.N_waves)], n_frame, name, call_type, Ria_type);
        ...
        function z = capon_minimum_variance_implementation(h,data_cube,apod_matrix,progress, n_frame, name, call_type, Ria_type)
                ...
                for k = 1:N
                    if (sum(abs(rf_data(k,:))>eps) == 0)
                        ...
                        if call_type == 2
                            ...
                            if call_type == 1 % save R and Ria
                                % R
                                R = (R_sub + I_new*(h.regCoef/L_new)*trace(R_sub));
                                
                                % Paths
                                R_path = strcat(image_dir,'\R\',num2str(n_frame),'_',num2str(e),'_',num2str(k),'.mat');
                                Ria_path = strcat(image_dir,'\Ria\',num2str(n_frame),'_',num2str(e),'_',num2str(k),'.mat');
